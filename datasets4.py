...
class SimplePCQM4MDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, (int, np.integer)):
            ...
            g['structure_feat_float'] = np.stack(
                [
            ...
            # g['structure_feat_float'] = np.concatenate((angle, dist, mid), axis=-1)
            g['structure_feat_cate'] = np.zeros((num_atom, num_atom, 1), dtype='int64')
            # g['atom_feat_float'] = np.concatenate((g['atom_feat_float'], g['xyz']), axis=-1)
            g['atom_feat_float'] = np.concatenate(
                [
                    g['atom_feat_float'],
                    g['atom_feat_cate'][:, 4:5].astype('float32'),
                    g['atom_feat_cate'][:, 5:6].astype('float32'),
                    g['atom_feat_cate'][:, 12:13].astype('float32'),
                    (g['atom_feat_cate'][:, 9:10] + g['atom_feat_cate'][:, 10:11]).astype('float32'),
                ]
                , axis=-1)
            g['atom_feat_cate'] = np.zeros((num_atom, 1), dtype='int64')
