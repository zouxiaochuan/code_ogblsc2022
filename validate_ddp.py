...
run_id = 'dropout0.1_decay1_0.97_h16s16_hidden256_finetune_en_2e-5_dropin_l16'
...
def main(rank, num_processes):
    ...
    scores = np.load('models_valid/dropout_decay1_0.97_h16_hidden256_fastedge_int8_bondreverse_sdfxyz_valid-train/pred_060.npy')
    scores = scores[:, None]

    dataset_train = datasets.SimplePCQM4MDataset(
        path=config['middle_data_path'], split_name='train', rotate=True, path_atom_map=None, data_path_name='data', extra_data=scores)
    dataset_test = datasets.SimplePCQM4MDataset(
        path=config['middle_data_path'], split_name='valid', rotate=False, path_atom_map=None, data_path_name='data', extra_data=scores)
