...
class SimplePCQM4MDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(idx, (int, np.integer)):
            ...
            # print(g['xyz'])
            # g['xyz'] = np.zeros((num_atom, 3), dtype='float32')
            ...
            # if dist.sum() == 0:
            #     g['structure_feat_float'] = g['predict_pair_dist'][:, :, None]
            #     pass
            # if 'predict_pair_dist' in g:
            # if False:
            #     g['structure_feat_float'] = g['predict_pair_dist'][:, :, None]
            # else:
...
def collate_fn(graph_list):
    ...
    if 'predict_pair_dist' in graph_list[0]:
        predict_pair_dist = common_utils.collate_map([g['predict_pair_dist'][:, :, None] for g in graph_list])
        pass
    ...
    if 'predict_pair_dist' in graph_list[0]:
        result_dict['predict_pair_dist'] = torch.from_numpy(predict_pair_dist)
        pass
