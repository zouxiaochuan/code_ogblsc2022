    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attention_scores = attention_scores + \
                (torch.diag(torch.ones(attention_scores.shape[-1], device=attention_scores.device)) * -10000.0)[None, None, :, :]
...
class TransformerLayer(nn.Module):
    def __init__(self, hidden_size, intermediate_size, attention_head_size, num_attention_heads,
        ...
        # self.intermediate = nn.Linear(hidden_size, 64)
        self.intermediate_act_fn = nn.GELU()
        # self.dense_output = nn.Linear(attention_head_size*attention_head_size*num_attention_heads, hidden_size)
    ...
    def forward(
        ...
        self_attention_outputs, attention_scores = self.attention(
            hidden_states,
            attention_mask,
            structure_matrix=structure_matrix,
        )
        ...
        # output = output.reshape(output.shape[0], output.shape[1], self.num_attention_heads, self.attention_head_size)
